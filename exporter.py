...
class BalenaCollector(object):
    ...
    def collect(self):
        gauge = GaugeMetricFamily("balena_devices_online", "Devices by status", labels=["fleet_name"])

        for fleet_id in get_balena_fleets():
            fleet_name, device_online_count = get_fleet_metrics(str(fleet_id))
